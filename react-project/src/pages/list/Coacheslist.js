...
import { Table, Space, Button, Avatar, Popconfirm, message, Modal, Form, Input } from 'antd';
...
export default class Studentlist extends Component {
    ...
    onFinish = async (values) => {
        try {
            ...
            const { success, msg } = await api.coaches.updateCoaches(values = { data: values, _id: CoachesId });
            if (success) {
                message.success(msg)
            }
            this.getallCoaches();
    ...
    async getallCoaches(parmas) {
        try {
            ...
            let { rows, totalCount, pageNumber } = await api.coaches.getCoaches(parmas = { pageSize: 10, pageNumber: 1, _id: gym._id });
            console.log(rows);
            this.setState({ data: rows, totalCount, current: pageNumber, gymInfo: gym })
        } catch (error) {
            console.log('错误=====================================');
        }
    }
